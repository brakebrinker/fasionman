...
class Msie
{
    ...
    public function testReader($file, $seek = 0, $limit = 2)
    {
        $skipLine = false;
        $iterator = 2;
        $tstart = microtime(1);
    ...
    private function importCategories($filename = '', $seek = 0)
    {
        ...
        $reader->read(array(
            'file' => $file,
            'seek' => $seek,
        ), function ($reader, $csv) use (&$i, &$out, $subDelimeter, $stepLimit, $isDebug) {
            $i++;
            $out['rows']++;
            $this->modx->error->reset();
            $categories = array();
            ...
            if (!$categoryId = $this->getCategoryIdByName($csv[0], true, false)) {
                $categoryId = $this->createCategory($csv[0], 0);
            }
